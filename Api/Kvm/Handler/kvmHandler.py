...
class VmsHandler(RESTfulHandler):
    ...
    def get(self, ident):
        ...
        if username == 'admin':
            ...
            KvmlistObj = self.db.query(KvmList).filter(KvmList.ProjectsName==self.get_secure_cookie('results'))
            totalquery = self.db.query(KvmList).filter(KvmList.ProjectsName.in_(project_name)).all()
            if searchKey:
                KvmlistObj = KvmlistObj.filter(or_(KvmList.CloudName.like('%%%s%%' % searchKey),
                                                   KvmList.Ip.like('%%%s%%' % searchKey),
                                                   KvmList.MainHost.like('%%%s%%' % searchKey)))
            serverTask = KvmlistObj.order_by(desc(KvmList.CreateTime)).limit(pagesize).offset((page - 1) * pagesize).all()
            d = dict((p.ProId,p.ProName) for p in project)
            self.Result['total'] = len(totalquery)
            self.Result['defaults'] = self.get_secure_cookie('results')
            self.Result['projects'] = d
            self.Result['username'] = username
            self.Result['rows'] = map(lambda obj: obj.toDict(), serverTask)
            self.finish(self.Result)
    ...
    def post(self,ident=0):
        ...
        if memory <= 3:
            ...
            print allocated_cpu_now,pro.Ccpu,allocated_memory_now,pro.Cmemory
...
class ProjectlistHandler(RESTfulHandler):
    ...
    def get(self):
        ...
        if username == "admin":
            ...
            pro_id = [ i.ProjectId for i in Pro ]
            project = self.db.query(ProjectsList).filter(ProjectsList.ProId.in_(pro_id)).all()
            projectlist = [p.ProName for p in project]
    ...
    def get(self, ident):
        if ident:
            ...
            if username == 'admin':
                ...
                project = self.db.query(ProjectsList).filter(ProjectsList.ProId.in_(pro_id)).all()
                project_name = [p.ProName for p in project]
                SecretKeylistObj = self.db.query(SecretKeyList).filter(SecretKeyList.ProjectsName==self.get_secure_cookie('results'))
                totalquery = self.db.query(SecretKeyList).filter(SecretKeyList.ProjectsName.in_(project_name)).all()
                if searchKey:
                     SecretKeylistObj = SecretKeylistObj.filter(or_(SecretKeyList.SecretkeyName.like('%%%s%%' % searchKey),
                                                                    SecretKeyList.CreatePerson.like('%%%s%%' % searchKey)))
                serverTask = SecretKeylistObj.order_by(desc(SecretKeyList.CreateTime)).limit(pagesize).offset((page - 1) * pagesize).all()
                d = dict((p.ProId,p.ProName) for p in project)
                self.Result['total'] = len(totalquery)
                self.Result['defaults'] = self.get_secure_cookie('results')
    ...
    def get(self, ident):
        if ident:
            ...
            if username == 'admin':
                ...
                project = self.db.query(ProjectsList).filter(ProjectsList.ProId.in_(pro_id)).all()
                project_name = [p.ProName for p in project]
                clouddisklistObj = self.db.query(CloudDiskList).filter(CloudDiskList.ProjectsName==self.get_secure_cookie('results'))
                totalquery = self.db.query(CloudDiskList).filter(CloudDiskList.ProjectsName.in_(project_name)).all()
                if searchKey:
                    clouddisklistObj = clouddisklistObj.filter(or_(CloudDiskList.ImageName.like('%%%s%%' % searchKey)))
                serverTask = clouddisklistObj.order_by(desc(CloudDiskList.CloudDiskId)).limit(pagesize).offset((page - 1) * pagesize).all()
                d = dict((p.ProId,p.ProName) for p in project)
...
class ResourceHandler(RESTfulHandler):
    ...
    def get(self,ident=1):
        ...
        if username == "admin":
            ...
            pro = self.db.query(ProjectsList).filter(ProjectsList.ProId==ident).first()
            self.Result['ProName'] = pro.ProName
            self.Result['allocate_cpu'] = pro.Vcpu
            self.Result['undistributed_cpu'] = pro.Ccpu - pro.Vcpu
            self.Result['allocate_mem'] = pro.Vmemory
            self.Result['undistributed_mem'] = pro.Cmemory - pro.Vmemory
        self.finish(self.Result)


#获取当前登录用户信息
@urlmap(r'/getinfo\/?([0-9]*)')
class GetInfoHandler(RESTfulHandler):
    @web.asynchronous
    def get(self,ident=0):
        ...
        if username != "admin":
            ...
            self.Result['username'] = username
            self.Result['projects'] = d
            self.Result['defaults'] = d[int(ident)]
            self.Result['results'] = self.db.query(ProjectsList).filter(ProjectsList.ProId==ident).first().ProName
            self.set_secure_cookie('results', self.Result['results'], expires_days=1)
        else:
            self.Result['username'] = username
        self.finish(self.Result)
